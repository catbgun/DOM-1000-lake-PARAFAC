...
#nrow(pca1x)
#pca1x <- na.omit(pca1)
pca1y <- subset(pca1, select=-c(Region.4))
region <- pca1[, 1]

#Logtransform parameters and change names to "_log"
ell <- log(pca1y+15)
names(ell) <- paste0(names(ell), "_log")
elli <- subset(ell, select=-c(X2019_ANCorg_log, del_ANCorg_log, X2019_ANC_log, del_ANCorg_log, del_TON_log, 
                              del_H_log, del_TOC_log, del_pH_log, del_SO4_log, dmax.C_log, altitude_log, del_SiO2_log, LakeTemp_19_log, Avrenning_log, X2019_Mg_log))
